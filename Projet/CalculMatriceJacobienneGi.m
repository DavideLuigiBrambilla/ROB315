function [OJv_Gi, OJ_wi] = CalculMatriceJacobienneGi(alpha, d, theta, r, x_G, y_G, z_G)
    ...
    for i = 1:size(J,2)
        ...
        OJ_Oi(:,1:i,i) = J(:,1:i);
        %Vecteur OiGi exprimé dans Ri
        iOiGi = [x_G(i) y_G(i) z_G(i)]';
        %Matrice de rotation ROi du repère R0 à Ri
        g_elem = CalculTransformationElem(alpha(i), d(i), theta(i), r(i));
        %Matrice de transformation élémentaire de passage de C(i-1)
        g_0i = g_0i*g_elem;
        ROi(:,:,i) = g_0i(1:3,1:3);
        %Vecteur OEGi dans RO à  calculer: OEGi = OEOi + OiGi
        %Vecteur OiGi exprimé dans R0
        OOiGi = ROi(:,:,i)*iOiGi;
        %Vecteur OEOi exprimé dans R0: OEOi = OEO0 + O0Oi
        O0Oi(:,:,i) = g_0i(1:3,4);
        OEOi = OEO0 + O0Oi(:,:,i);
        OOEGi = OEOi + OOiGi;
